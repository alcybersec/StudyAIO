...
class TestUploadsScopedByUser:
    ...
    async def test_upload_passes_user_id_to_pipeline(self, async_client):
        ...
        with patch("app.api.uploads.run_pipeline") as mock_pipeline:
            ...
            with patch("app.api.uploads.artifact_service.get_artifact", new_callable=AsyncMock) as mock_get:
...
class TestDashboardScopedByUser:
    ...
    async def test_dashboard_passes_user_id_to_services(self, async_client):
        ...
        with patch("app.api.dashboard.review_service.count_pending_reviews", new_callable=AsyncMock, return_value=0) as mock_review, \
             patch("app.api.dashboard.pipeline_service.get_recent_activity", new_callable=AsyncMock, return_value=[]) as mock_activity, \
             patch("app.api.dashboard.course_service.list_courses_with_stats", new_callable=AsyncMock, return_value=[]) as mock_courses, \
             patch("app.api.dashboard.srs_service.get_global_study_stats", new_callable=AsyncMock, return_value={"total": 0, "due_today": 0, "mastered": 0, "learning": 0, "new": 0}) as mock_stats, \
             patch("app.api.dashboard.srs_service.get_per_course_due_counts", new_callable=AsyncMock, return_value=[]) as mock_due, \
             patch("app.api.dashboard.exam_service.list_exams", new_callable=AsyncMock, return_value=[]) as mock_exams, \
             patch("app.api.dashboard.streak_service.get_streak", new_callable=AsyncMock, return_value={"current_streak": 0, "longest_streak": 0, "last_study_date": None}) as mock_streak, \
             patch("app.api.dashboard.courseops_service.get_upcoming_deadlines_all_courses", new_callable=AsyncMock, return_value=[]):
