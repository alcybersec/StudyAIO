"""Ollama adapter — calls a local Ollama server for AI operations."""
...
_SUMMARY_NUM_PREDICT = 8192


class OllamaAdapter(AgentAdapter):
    """Calls an Ollama server via the official Python SDK."""

    def __init__(self, base_url: str = "", model: str = ""):
        self._base_url = base_url or get_effective_setting("ollama_base_url")
        self._model = model or get_effective_setting("ollama_model")

    async def _call_api(self, prompt: str, num_predict: int = _DEFAULT_NUM_PREDICT) -> str:
    ...
    async def classify_lecture(
        ...
        """Classify a lecture by calling Ollama."""
...
"""
        ...
        result_text = await self._call_api(prompt, num_predict=_SUMMARY_NUM_PREDICT)
    ...
    async def generate_flashcards(
        ...
        """Generate flashcards by calling Ollama."""
        from pathlib import Path
...
"""
    ...
    async def generate_quiz(
        ...
        """Generate quiz questions by calling Ollama."""
        from pathlib import Path
...
"""
    ...
    async def answer_question(self, question: str, context_chunks: list[dict]) -> AnswerResult:
        """Answer a question by calling Ollama."""
        from pathlib import Path
...
{{
  ...
  "citations": [{{"ref": 1, "chunk_id": "", "text_snippet": "brief quote", "course_code": "CSIT302", "week": 5, "page_ref": 1}}]
    ...
    async def extract_course_ops(
        ...
        """Extract course logistics by calling Ollama."""
        from pathlib import Path
...
"""
    ...
    async def extract_concepts(
        ...
        """Extract concepts and relationships by calling Ollama."""
        from pathlib import Path
