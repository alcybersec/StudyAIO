...
| S.2 | Session revocation holds in self-hosted mode (#16) | ✅ Done | `get_current_user_or_default` caught every `AuthenticationError` in self-hosted mode and fell back to the shared default admin, so a token revoked by a password reset was silently downgraded to that (admin) identity instead of rejected. New `SessionRevokedError(AuthenticationError)` raised by the revocation checks in `get_current_user` and `POST /api/auth/refresh`; the self-hosted fallback re-raises it and still falls back for every other auth failure. `get_optional_user` deliberately keeps returning `None` — it grants no identity. Inherits the 401 handler via MRO. 7 new tests. |
| S.3 | Make the change-password sign-out deliberate (#15) | ✅ Done | `change_password` and `disable_mfa` stamp `tokens_valid_from`, so the caller's own cookies died the moment they succeeded — the next request 401'd with no explanation, every time. Kept the sign-out (option (b) in the issue: it is the right posture, and re-minting a token in the same second would be rejected by `is_token_invalidated`'s `iat <= cutoff`) but made it explicit: both endpoints now clear the auth cookies and return `SessionEndedResponse` (`session_ended: true`). Frontend `useSessionHandoff()` drops the cached user and navigates to `/login?reason=...` in one update (doing it in two lets ProtectedRoute win the race and lose the reason); LoginPage renders the explanation. 6 backend + 7 vitest tests. |
...
| # | Task | Status | Notes |
|---|------|--------|-------|
| BR.1 | Golden summary test actually guards the prompt | ✅ Done | `tests/golden/test_summary_structure.py` asserted the v1 8-section format against its own hand-written fixture, so it passed no matter what `prompts/summarize.txt` said. New `TestPromptContract` re-derives the required section list *from the prompt file* each run and fails when the two disagree; fixture rewritten to the real v2.0 output (Overview → Key Concepts → Definitions → Diagrams & Visual Descriptions → Code Examples → Formulas & Algorithms → Key Takeaways → Connections, `*Sources: … Version: N.*` footer). The Key Concepts check now asserts `###`/`####` subsections rather than bullets, matching v2.0's "no flat bullet dump" rule. Verified by mutating the prompt and watching the suite go red. 16 → 32 tests. |
| BR.2 | Remove the magic-link stubs | ✅ Done | `POST /api/auth/magic-link` sent a *password reset* email and `GET /api/auth/magic/{token}` returned "not yet fully implemented" without authenticating — live, auth-shaped, unreferenced by the UI. Both endpoints and `MagicLinkRequest` deleted. The `MagicLink` model stays: password reset and email verification use it. |
| BR.3 | Invite-gated registration | ✅ Done | New `REGISTRATION_MODE` = `open` (default) \| `invite` \| `closed`, enforced in `POST /api/auth/register` — not just advertised via `/api/auth/config`. `invite_codes` table + `invite_service`: codes like `BETA-7F3KQ2MN` (alphabet excludes `0/O/1/I/L`), `max_uses`, expiry, revocation. Redemption takes `SELECT … FOR UPDATE` so two simultaneous sign-ups cannot both spend the last use, and returns an identical error for unknown/spent/expired codes so a guesser learns nothing. The code is redeemed *before* the user is created, so a bad code costs nothing. `users.invite_code_id` records which invite an account used. Admin CRUD at `/api/admin/invites`. Migration `y5z6a7b8c9d0`. |
| BR.4 | Account deletion + data export | ✅ Done | `DELETE /api/auth/account` (re-auth by password, or by typed username for OAuth-only accounts) and `GET /api/auth/account/export`. Deletion order comes from `Base.metadata.sorted_tables` reversed rather than a hand-kept list, and **every** table must be classified as user-scoped or explicitly global — `test_account_deletion.py::test_every_table_is_classified` fails the build if a new table is neither, so a future table cannot silently escape deletion. Storage blobs are enumerated from the DB and purged first (keys are not user-namespaced); added `delete_prefix` to both storage backends. Export reuses the same predicates, minus credential columns, so "download my data" and "delete my data" cannot disagree. |
| BR.5 | Error monitoring (Sentry) | ✅ Done | `app/core/observability.py` (`init_sentry`) wired into API and worker; `src/lib/monitoring.ts` (`@sentry/react`, dynamic import → own chunk) wired into `main.tsx`, `ErrorBoundary` and `RouteErrorBoundary`. Entirely inert when `SENTRY_DSN`/`VITE_SENTRY_DSN` is empty, degrades to a no-op if the SDK is missing or ad-blocked, and never raises. `send_default_pii=False` plus a `before_send` scrubber that filters credential headers, drops cookies, and strips `?token=` from URLs — reset and verification links are bearer credentials and must not land in Sentry. |
| BR.6 | Preflight warns about beta footguns | ✅ Done | `scripts/preflight-check.sh` now errors when SMTP is unconfigured in SaaS mode (password resets silently do nothing — a locked-out tester has no way back in) and warns when `REGISTRATION_MODE=open` with `SELF_HOSTED=false`. |
